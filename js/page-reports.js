...
function renderReportCard(out, d, readOnly) {
  ...
  const acc = d.access || { active: true };
  const lockStyle = "background:#ECEFF1;color:#546E7A;box-shadow:none";
  let pdfBtn;
  if (acc.active) {
    pdfBtn = `<button class="btn btn-pdf btn-sm" onclick="window.open('${pdfUrl}','_blank')">
    ...
    pdfBtn = `<button class="btn btn-sm" style="${lockStyle}" onclick="showPage('parent-subscription')">🔒 Report PDF - Parent access required</button>`;
  } else if (currentUser.role === "admin") {
    pdfBtn = `<button class="btn btn-sm" style="${lockStyle}" onclick="openAccessModal(${sid})">🔒 Report PDF - Parent access required</button>`;
  } else {
    pdfBtn = `<button class="btn btn-sm" style="${lockStyle};cursor:not-allowed" disabled title="Ask the school admin to activate this student's parent access">🔒 Report PDF - Parent access required</button>`;
  }
